...
export default function App() {
  ...
  const [tab, setTab] = useState<'record' | 'contrast'>('contrast');

  useEffect(() => {
    fetch('/api/audits').then(r => r.json()).then(setItems);
  }, []);
  useEffect(() => {
    setStatus('Loading');
    fetch('/api/audits/' + selected)
      .then(r => r.json())
      .then((value: Row) => {
        setRow(value);
  ...
  async function analyze() {
    if (!row) return;
    setStatus('Analyzing');
    const response = await fetch('/api/audits/' + row.id + '/analyze', {
      method: 'POST',
      headers: {'content-type': 'application/json'},
